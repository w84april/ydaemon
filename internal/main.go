...
func Initialize(chainID uint64) {
	...
	**********************************************************************************************/
	...
	allHarvestPerStrategyPerVault := map[ethcommon.Address]map[ethcommon.Address][]vaults.THarvest{}
	count := 0
	for _, v := range harvests {
		AllHarvests[v.Vault] = append(AllHarvests[v.Vault], v)
		if allHarvestPerStrategyPerVault[v.Vault] == nil {
			allHarvestPerStrategyPerVault[v.Vault] = map[ethcommon.Address][]vaults.THarvest{}
		}
	...
	// 	}
	// }
	curveSTETHVault := common.HexToAddress(`0x5B8C556B8b2a78696F0B9B830B3d67623122E270`).ToAddress()
	curveSTETHStrategy := common.HexToAddress(`0xaBec96AC9CdC6863446657431DD32F73445E80b1`).ToAddress()
